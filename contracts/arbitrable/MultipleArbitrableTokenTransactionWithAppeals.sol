...
// import "@kleros/kleros-interaction/contracts/standard/arbitration/IArbitrable.sol";
...
//    */
//   function reimburse(uint256 _transactionID, uint256 _amountReimbursed) public {
//     Transaction storage transaction = transactions[_transactionID];
...
//     require(
//       transaction.token.transfer(transaction.sender, _amountReimbursed),
...
//    */
//   function submitEvidence(uint256 _transactionID, string _evidence) public {
//     Transaction storage transaction = transactions[_transactionID];
//     require(
...
//    */
//   function fundAppeal(uint256 _transactionID, Party _side) public payable {
//     Transaction storage transaction = transactions[_transactionID];
...
//     require(
//       arbitrator.disputeStatus(transaction.disputeId) == Arbitrator.DisputeStatus.Appealable,
...
//     uint256 winner = arbitrator.currentRuling(transaction.disputeId);
//     uint256 multiplier;
...
//     uint256 appealCost = arbitrator.appealCost(transaction.disputeId, arbitratorExtraData);
...
//    * If the appeal was fully funded, sends the fee stake rewards and reimbursements proportional to the contributions
//    * made to the winner of a dispute.
...
//     } else if (transaction.ruling == uint256(Party.None)) {
//       // Reimburse unspent fees proportionally if there is no winner and loser.
//       uint256 rewardSender = round.paidFees[uint256(Party.Sender)] > 0
//         ? (round.contributions[_beneficiary][uint256(Party.Sender)] * round.feeRewards) /
//           (round.paidFees[uint256(Party.Sender)] + round.paidFees[uint256(Party.Receiver)])
//         : 0;
//       uint256 rewardReceiver = round.paidFees[uint256(Party.Receiver)] > 0
//         ? (round.contributions[_beneficiary][uint256(Party.Receiver)] * round.feeRewards) /
//           (round.paidFees[uint256(Party.Sender)] + round.paidFees[uint256(Party.Receiver)])
//         : 0;
...
//         ? (round.contributions[_beneficiary][transaction.ruling] * round.feeRewards) /
//           round.paidFees[transaction.ruling]
//         : 0;
//     }
//     round.contributions[_beneficiary][uint256(Party.Sender)] = 0;
//     round.contributions[_beneficiary][uint256(Party.Receiver)] = 0;

//     _beneficiary.send(reward); // It is the user responsibility to accept ETH.
//   }

//   /** @dev Withdraws contributions of multiple appeal rounds at once. This function is O(n) where
//    * n is the number of rounds. This could exceed the gas limit, therefore this function should be
//    * used only as a utility and not be relied upon by other contracts.
//    *  @param _beneficiary The address that made contributions.
//    *  @param _transactionID The ID of the associated transaction.
...
//   ) public {
//     Transaction storage transaction = transactions[_transactionID];
//     for (uint256 i = _cursor; i < transaction.rounds.length && (_count == 0 || i < _cursor + _count); i++)
...
//     if (round.hasPaid[uint256(Party.Sender)] == true) resultRuling = Party.Sender;
//     else if (round.hasPaid[uint256(Party.Receiver)] == true) resultRuling = Party.Receiver;

//     emit Ruling(Arbitrator(msg.sender), _disputeID, uint256(resultRuling));

//     executeRuling(transactionID, uint256(resultRuling));
//   }
...
//   /** @dev Returns the sum of withdrawable wei from appeal rounds. This function is O(n), where n
//    * is the number of rounds of the transaction. This could exceed the gas limit, therefore this
//    * function should only be used for interface display and not by other contracts.
//    *  @param _transactionID The index of the transaction.
//    *  @param _beneficiary The contributor for which to query.
//    *  @return The total amount of wei available to withdraw.
//    */
//   function amountWithdrawable(uint256 _transactionID, address _beneficiary) public view returns (uint256 total) {
//     Transaction storage transaction = transactions[_transactionID];
//     if (transaction.status != Status.Resolved) return total;

//     for (uint256 i = 0; i < transaction.rounds.length; i++) {
...
//       } else {
//         total += round.paidFees[uint256(transaction.ruling)] > 0
//           ? (round.contributions[_beneficiary][uint256(transaction.ruling)] * round.feeRewards) /
//             round.paidFees[uint256(transaction.ruling)]
//           : 0;
//       }
//     }
...
//   ) public view returns (uint256[3] contributions) {
//     Transaction storage transaction = transactions[_transactionID];
//     Round storage round = transaction.rounds[_round];
...
//    */
//   function getTransactionIDsByAddress(address _address) public view returns (uint256[] transactionIDs) {
//     uint256 count = 0;
