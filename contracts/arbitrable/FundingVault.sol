...
contract FundingVault is Arbitrable {
  ...
  uint256 public claimToWithdrawTime;
  uint256 public additionalTimeToWithdraw;
  uint256 public timeout;
  struct Milestone {
    uint256 amount; // The maximum amount which can be unlocked for this milestone.
    uint256 amountClaimed; // The current amount which is claimed.
    uint256 claimTime; // The time the current claim was made. Or 0 if it's not currently claimed.
    bool disputed; // True if a dispute has been raised.
    uint256 feeTeam; // Arbitration fee paid by the team.
    uint256 feeHolders; // Arbitration fee paid by token holders.
    ...
    uint256 lastTotalFeePayment; // Time of the last total fee payment, useful for timeouts.
  ...
  mapping(uint256 => uint256) public disputeIDToMilstoneID; // Map (disputeID => milestoneID).
  uint256 ousterID; //The ID of the milestone created at construction. To be disputed when the funders claim the team is not doing their job.
  ...
  constructor(
    ...
    uint256 _claimToWithdrawTime,
    uint256 _additionalTimeToWithdraw,
    uint256 _timeout
    ...
    ousterID =
      milestones.push(
        Milestone({ //Create a base milestone to be disputed when the funders claim the team is not doing their job.
          amount: 0,
          amountClaimed: 0,
          claimTime: 0,
          disputed: false,
   ...
   */
  function fundMilestone() public payable returns (uint256 milestoneID) {
  ...
  modifier isNotOuster(uint256 _milestoneID) {
   ...
   */
  function claimMilestone(uint256 _milestoneID, uint256 _amount) public isNotOuster(_milestoneID) {
   ...
   *  @param _milestoneID The ID of the milestone.
   */
  function makeVoteToken(uint256 _milestoneID) public {
    Milestone storage milestone = milestones[_milestoneID];
    // The milestone is currently claimed by the team, unless this is the ouster.
    if (ousterID != _milestoneID) {
      require(milestone.claimTime != 0, "The milestone must be claimed by the team.");
    }
   ...
   */
  function payDisputeFeeByHolders(uint256 _milestoneID) public payable {
    Milestone storage milestone = milestones[_milestoneID];
    ...
    if (milestone.payerForHolders == 0x0) milestone.payerForHolders = msg.sender;

    if (milestone.feeTeam >= arbitrationCost) {
      // Enough has been paid by all sides.
      createDispute(_milestoneID, arbitrationCost);
    } else if (milestone.lastTotalFeePayment == 0) {
      // First time the fee is paid.
      milestone.lastTotalFeePayment = now;
    } else if (milestone.lastTotalFeePaymentIsTeam) {
      // The team was the last one who had paid entirely.
   ...
   *  @param _milestoneID The milestone which is disputed.
   */
  function payDisputeFeeByTeam(uint256 _milestoneID) public payable {
    Milestone storage milestone = milestones[_milestoneID];
    uint256 arbitrationCost = arbitrator.arbitrationCost(arbitratorExtraData);
    require(msg.sender == team, "Can only be called by the team.");
    require(!milestone.disputed, "The milestone cannot be already disputed.");
    ...
    if (milestone.feeHolders >= arbitrationCost) {
      // Enough has been paid by all sides.
      createDispute(_milestoneID, arbitrationCost);
    } else if (milestone.lastTotalFeePayment == 0) {
      // First time the fee is paid.
      ...
      // The holders were the last ones who had paid entirely.
      milestone.lastTotalFeePaymentIsTeam = true;
      milestone.lastTotalFeePayment = now;
    }
  }
   ...
   *  @param _milestoneID The milestone which is disputed.
   */
  function withdraw(uint256 _milestoneID) public isNotOuster(_milestoneID) {
    Milestone storage milestone = milestones[_milestoneID];
    require(msg.sender == team, "Can only be called by the team.");
    ...
    require(
      ...
      (now - milestone.claimTime) >
        claimToWithdrawTime +
          (additionalTimeToWithdraw * milestone.voteToken.balanceOf(this)) /
          (1000 * milestone.voteToken.totalSupply()),
   ...
   *  @param _milestoneID The milestone which is disputed.
   */
  function timeoutByTeam(uint256 _milestoneID) public {
    Milestone storage milestone = milestones[_milestoneID];
    require(msg.sender == team, "Can only be called by the team.");
   ...
   *  @param _milestoneID The milestone which is disputed.
   */
  function timeoutByHolders(uint256 _milestoneID) public {
    Milestone storage milestone = milestones[_milestoneID];
    require(!milestone.lastTotalFeePaymentIsTeam, "Team wasn't the last to pay.");
    require(now - milestone.lastTotalFeePayment > timeout, "Timeout has not passed.");
  ...
  function executeRuling(uint256 _disputeID, uint256 _ruling) internal {
    ...
    uint256 _milestoneID = disputeIDToMilstoneID[_disputeID];
    if (_ruling == TEAM_WINS) {
      ...
      milestone.payerForHolders = 0x0;
      if (ousterID == _milestoneID) {
        //if this is the ouster milestone
